...
function detectBuildTool(): BuildTool {
  // 1. ✅ Next.js 检测（结合构建器）
  if (
    hasProjectFile('next.config.js') ||
    hasProjectFile('next.config.mjs') ||
    hasProjectFile('next.config.ts')
  ) {
    try {
  ...
  if (
    ...
    hasProjectFile('webpack.config.ts')
  ) {
    return 'webpack';
  }
  ...
  // 4. ✅ Rollup
  if (
    hasProjectFile('rollup.config.js') ||
    hasProjectFile('rollup.config.ts')
  ) {
    return 'rollup';
  }
  ...
  if (
    hasProjectFile('turbopack.config.js') ||
    hasProjectFile('turbopack.config.ts')
  ) {
    return 'turbopack';
  }
...
export function initProjectStructure(defaultFramework?: FrameworkType): void {
  if (!instance) {
...
export function getProjectStructure(): ProjectStructure {
  if (!instance) {
    throw new Error(
      'ProjectStructure is not initialized. Please call initProjectStructure() first.'
    );
  }
  return instance;
}
